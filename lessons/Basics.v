...
Proof.
  intros. rewrite <- mult_n_O. rewrite <- mult_n_O. simpl. reflexivity.
Qed.
...
Proof.
  intros. rewrite <- mult_n_Sm. rewrite <- mult_n_O. simpl. reflexivity.
Qed.

Theorem plus_1_neq_0 : forall n : nat, (n + 1) =? 0 = false.
Proof.
  intros. destruct n as [| m] eqn : E.
  - simpl. reflexivity.
  - simpl. reflexivity.
Qed.
...
(*Fixpoint addd (a n m : nat) : nat :=
  match (eqb a n) with
  | true => m
  | false => addd a (S n) (S m)
  end.*)
...
Theorem lower_grade_lowers : forall (g : grade),
  grade_comparison (Grade F Minus) g = Lt -> grade_comparison (lower_grade g) g = Lt.
Proof.
  intros. destruct g. destruct l.
  - simpl. destruct m. reflexivity. reflexivity. reflexivity.
  - simpl. destruct m. reflexivity. reflexivity. reflexivity.
  - simpl. destruct m. reflexivity. reflexivity. reflexivity.
  - simpl. destruct m. reflexivity. reflexivity. reflexivity.
  - simpl. destruct m. 
...
Proof.
  intros. rewrite -> apply_late_policy_unfold. rewrite -> H. rewrite -> H0.
  reflexivity.   
Qed.
End LateDays.
