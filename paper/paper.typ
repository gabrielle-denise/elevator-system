...
the model imposes no constraint on _which_ commands arrive or in what order. A hostile or
...
the system can never enter an unsafe state. The complete Rocq development is included
alongside this paper in the file #raw("elevator_FINAL.v").
...
- *No liveness (by design)*: The model does not prove that the elevator will _eventually_
  reach any requested floor. The root cause is that the model is purely command-driven:
  `transition s cmd` produces a new state only when the environment supplies a command,
  ...
  reachability invariant chosen for Property 6 is provable within the current model and
  fulfills the paper's primary goal of _safety_ verification: the elevator never enters an
  unsafe state regardless of what commands the environment sends.
